...
.total-card{padding:18px;border-radius:20px;background:linear-gradient(135deg,rgba(30,190,110,.12),rgba(45,125,255,.12));border:1px solid rgba(127,127,127,.16);margin:14px 0}
...
def score_explanations(row, hist):
    ...
    r5 = float(x["ret5"]) * 100 if pd.notna(x["ret5"]) else 0.0
    r20 = float(x["ret20"]) * 100 if pd.notna(x["ret20"]) else 0.0
    ...
    if close > ma5 > ma10 > ma20:
        ...
        trend_note = f"站上5日、10日线，但20日线尚未完全走顺；近5天{above5}天站上5日线。"
    elif close > ma10:
        trend_note = f"站上10日线，但短中期均线仍纠缠；近5天{above5}天站上5日线。"
    else:
        trend_note = "价格仍在关键均线下方，趋势尚未确认。"

    if breakout and vr >= 1.25:
        volume_note = f"放量突破，成交量约为20日均量的{vr:.2f}倍。"
    elif vr < .75:
        volume_note = f"量能偏弱，约为20日均量的{vr:.2f}倍。"
    elif vr > 1.35:
        volume_note = f"量能明显放大至20日均量的{vr:.2f}倍，需结合涨跌判断。"
    else:
        volume_note = f"量能正常，约为20日均量的{vr:.2f}倍。"

    strength_note = f"近5日{r5:+.1f}%，近20日{r20:+.1f}%" + ("，接近或突破20日新高。" if breakout else "。")
    risk_note = f"距离20日线{dist:+.1f}%。" + ("偏离较大，追高风险上升。" if dist > 12 else "与中期均线距离可控。")
    market_note = f"市场环境得分{int(row['市场'])}/10。" + ("基准偏强。" if row["市场"] >= 8 else "大盘一般，建议控制总体仓位。" if row["市场"] <= 4 else "大盘环境中性。")
    return trend_note, volume_note, strength_note, risk_note, market_note

status_text, mins_left = market_status()
now = datetime.now()
st.markdown(
    f'<div class="hero"><div class="hero-kicker">TrendOS V3.1 · {now:%Y-%m-%d %H:%M:%S}</div>'
    f'<div class="hero-title">15:00 前，只看今天该怎么做</div>'
    f'<div class="hero-sub">{status_text}{f" · 距离收盘约 {mins_left} 分钟" if mins_left is not None else ""} · 盘中信号为临时日K</div></div>',
    unsafe_allow_html=True,
)
...
with st.expander("⚙️ 系统设置与板块方向", expanded=False):
    ...
    st.markdown("### 板块方向")
    selected_pairs = []
    group_tabs = st.tabs(list(SECTOR_CATALOG.keys()))
    for tab, (group, sectors) in zip(group_tabs, SECTOR_CATALOG.items()):
        with tab:
            cols = st.columns(2)
            for i, (name, code) in enumerate(sectors.items()):
                with cols[i % 2]:
                    checked = st.checkbox(
                        ...
                        key=f"sector_{group}_{name}",
                    )
                    if checked:
                        selected_pairs.append((name, code))

    st.markdown("---")
    custom_text = st.text_area(
        "自定义方向（每行：板块名,6位ETF代码）",
        value="",
        height=100,
        placeholder="例如：低空经济,159528\n传媒,512980",
    )
    ...
    st.caption(f"当前共选择 {len(selected_pairs)} 个方向。勾选后会直接进入实时排行。")
...
def dashboard():
    ...
    try:
        ...
        if not spot_map.empty and benchmark_code in spot_map.index:
            benchmark = merge_intraday_quote(benchmark, spot_map.loc[benchmark_code])
    except Exception:
        benchmark = None

    rows, histories = [], {}
    for name, code in selected_pairs:
        try:
            ...
            etf_name = ETF_UNIVERSE.get(code, name)
            rows.append({
                "板块":name,"ETF":etf_name,"代码":code,
            ...
            rows.append({
                "板块":name,"ETF":ETF_UNIVERSE.get(code,name),"代码":code,
    ...
    top = rank.iloc[0]
    headline = "出现可关注方向" if int(top["总分"]) >= 80 else "今天以观察为主"

    st.markdown(
        f'<div class="card"><div class="muted">今日总评</div>'
        f'<div style="font-size:30px;font-weight:800;margin:6px 0">{headline}</div>'
        f'<span class="pill good">{status_text}</span>'
        f'<div style="margin-top:12px">{top["板块"]}评分最高，但仍建议结合回踩或收盘确认。</div></div>',
        unsafe_allow_html=True,
    )
    ...
    m4.metric("达到试仓线",int((rank["总分"]>=70).sum()))
    ...
    with tabs[0]:
        ...
        for idx,row in candidates.iterrows():
            ...
            st.markdown(
                ...
                f'<div><div class="muted">#{idx+1} · {row["ETF"]} · {row["代码"]}</div>'
                f'<div style="font-size:24px;font-weight:800">{row["板块"]}</div>'
                f'<span class="pill {status_class(row["操作"])}">{row["操作"]}</span></div>'
                f'<div><div class="score">{int(row["总分"])}</div><div class="muted">综合评分</div></div></div>'
                f'<div style="margin-top:12px">{row["系统判断"]}{amount_text}</div></div>',
                unsafe_allow_html=True,
            )

    with tabs[1]:
        st.dataframe(
            rank[["板块","ETF","代码","最新价","涨跌幅%","MA5","MA10","MA20","总分","操作","系统判断"]],
    ...
    with tabs[2]:
        if holdings.empty:
            st.info("当前没有持仓。可在 data/holdings.csv 中填写 name,code,shares,cost_price。")
        else:
    ...
    with tabs[3]:
        ...
        if available.empty:
            ...
            fig.add_trace(go.Candlestick(
                x=data["date"],open=data["open"],high=data["high"],low=data["low"],close=data["close"],name="K线"
            ))
            for n in (5,10,20):
                fig.add_trace(go.Scatter(x=data["date"],y=data[f"ma{n}"],mode="lines",name=f"MA{n}"))
            ...
            total_label,total_class = score_level(total_score,100)
            st.markdown(
                f'<div class="total-card"><div class="muted">综合总分</div>'
                f'<div style="display:flex;justify-content:space-between;align-items:end">'
                f'<div style="font-size:46px;font-weight:800">{total_score}<span style="font-size:20px;opacity:.55"> / 100</span></div>'
                f'<span class="pill {total_class}">{total_label}</span></div></div>',
                unsafe_allow_html=True,
            )

            notes = score_explanations(row,hist)
            items = [
                ("趋势",int(row["趋势"]),40,notes[0]),
                ("量能",int(row["量能"]),20,notes[1]),
                ("强度",int(row["强度"]),20,notes[2]),
                ("风险安全",int(row["风险"]),10,notes[3]),
                ("市场",int(row["市场"]),10,notes[4]),
            ]

            boxes = []
            for name,value,maximum,note in items:
                label,css_class = score_level(value,maximum)
                boxes.append(
                    f'<div class="score-box">'
                    f'<div class="score-name">{name} · {value}/{maximum}</div>'
                    f'<div class="score-value">{value}</div>'
                    f'<span class="pill {css_class}">{label}</span>'
                    f'<div class="score-note" style="margin-top:9px">{note}</div>'
                    f'</div>'
                )
            html = '<div class="score-grid">' + ''.join(boxes) + '</div>'
            ...
            with st.expander("查看五项评分的详细计算逻辑"):
                st.markdown(
                    ...
                    f"**量能 {int(row['量能'])}/20**  \n{notes[1]}\n\n"
                    f"**强度 {int(row['强度'])}/20**  \n{notes[2]}\n\n"
                    f"**风险安全 {int(row['风险'])}/10**  \n{notes[3]}\n\n"
                    f"**市场 {int(row['市场'])}/10**  \n{notes[4]}"
                )

    with tabs[4]:
        st.markdown("""
- 勾选板块后会直接按对应ETF代码进入实时排行，不再依赖模糊关键词匹配。
- 自定义方向需填写“板块名,6位ETF代码”。
- 趋势40分、量能20分、强度20分、风险10分、市场10分。
""")
